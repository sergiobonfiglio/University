...
public final class Galois {
	private static final int TBL_DIM = 255;
	public static final int POLYNOMIAL = 0x1d;
	private static final Galois instance = new Galois();
	private int[] expTbl = new int[TBL_DIM * 2];	// 二重にもつことによりmul, div等を簡略化
	private int[] logTbl = new int[TBL_DIM + 1];
	...
	private void initGaloisTable() {
		int d = 1;
		for(int i = 0; i < TBL_DIM; i++) {
			expTbl[i] = expTbl[TBL_DIM + i] = d;
			logTbl[d] = i;
			d <<= 1;
			if((d & 0x100) != 0) {
				d = (d ^ POLYNOMIAL) & 0xff;
	...
	public int div(int a, int b) {
		return (a == 0)? 0 : expTbl[logTbl[a] - logTbl[b] + TBL_DIM];
	}
	...
	public int divExp(int a, int b) {
		return (a == 0)? 0 : expTbl[logTbl[a] - b + TBL_DIM];
	}
	...
	public int inv(int a) {
		return expTbl[TBL_DIM - logTbl[a]];
	}
